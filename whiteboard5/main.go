...
type CRDT struct {
	...
	Texts  map[string]string // Grow-Only Set (G-Set) for texts
}
...
// AddText adds a text to the CRDT
func (c *CRDT) AddText(id string, data string) {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.Texts[id] = data
}
...
// GetTexts returns a copy of the texts in the CRDT
func (c *CRDT) GetTexts() map[string]string {
	c.mu.Lock()
	defer c.mu.Unlock()
	copy := make(map[string]string)
	for id, data := range c.Texts {
		copy[id] = data
	}
	return copy
}
...
func saveSnapshot(id string, data string, isText bool) {
	key := fmt.Sprintf("%s:%t", id, isText)
...
func loadSnapshots() {
	...
	for _, key := range keys {
		if data, err := rdb.Get(ctx, key).Result(); err == nil {
			var isText bool
			fmt.Sscanf(key, "%*s:%t", &isText)
			if isText {
				crdt.AddText(key, data)
			} else {
				crdt.AddShape(key, data)
			}
		}
	}
}
...
func handlePeerConnection(conn net.Conn) {
	...
	for {
		...
		if message["type"] == "shape" {
			crdt.AddShape(message["id"].(string), message["data"].(string))
		} else if message["type"] == "text" {
			crdt.AddText(message["id"].(string), message["data"].(string))
		}
	}
}
...
func saveStateToRedis() {
	...
	for range ticker.C {
		for id, data := range crdt.GetShapes() {
			saveSnapshot(id, data, false)
		}
		for id, data := range crdt.GetTexts() {
			saveSnapshot(id, data, true)
		}
...
func handleConnections(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		messageType, ok3 := message["type"].(string)
		...
		if messageType == "shape" {
			crdt.AddShape(id, data)
		} else if messageType == "text" {
			crdt.AddText(id, data)
		}
